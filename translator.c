...
int parse_expression(FILE *fd, struct ast_node *node, storage_t res);

int add_oper(FILE *fd, struct ast_node *node, storage_t dest) {
    storage_t src1, src2;
    src1 = get_storage(fd);
    if(parse_expression(fd, node->value.expression.left, src1) == -1) {
        return -1;
    }
    src2 = get_storage(fd);
    if(parse_expression(fd, node->value.expression.right, src2) == -1) {
        return -1;
    }
...
int parse_expression(FILE *fd, struct ast_node *node, storage_t res) {
    ...
    case INTEGER_T:
        ...
        break;
    case IDENT_T:
        return load_ident(fd, res, node->value.str, 1);
        break;
    case EXPRESSION_T:
        switch(node->value.expression.operation) {
            case PLUS_OP:
                return add_oper(fd, node, res);
                break;
            default:
...
int parse_statement(FILE *fd, struct ast_node *node) {
    ...
    case REAL_T:
    case CAST_T:
        return parse_expression(fd, node, dummy);
        break;
    case FOR_T:
    case WHILE_T:
...
int parse_statement_list(FILE *fd, struct ast_node *node) {
    ...
    case AST_LIST_ELEMENT_T:
        for(struct ast_node *cur = node; cur != NULL; cur = cur->value.ast_list_element.next) {
            if(parse_statement(fd, node->value.ast_list_element.node) == -1) {
                return -1;
            }
        }
        break;
...
int parse_function_def_list(FILE *fd, struct ast_node *node) {
    ...
    case FUNCTION_T:
        ...
        break;
    default:
        printf("something else in function definition space!");
        break;
    }
    return 0;
}
